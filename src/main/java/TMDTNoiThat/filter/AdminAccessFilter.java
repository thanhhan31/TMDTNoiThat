...
@WebFilter(filterName = "adminAccess", urlPatterns = {"/admin/*"})
public class AdminAccessFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (user == null){
			...
			if (user.getUserRole().getId() != ConstantValue.ADMIN_ROLE) {
				resp.sendRedirect(req.getServletContext().getContextPath() + ConstantValue.ERROR_URL + "?msg=1");
			}
			else {

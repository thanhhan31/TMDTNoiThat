...
public class ProductMgrController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		productCount = psv.adminCountProducts(idCategory, searchName, status, outOfStock).intValue();
		
		while (currentPage * pagesize > productCount) {
			currentPage = currentPage - 1;
		}
		
		products = psv.adminGetProducts(idCategory, searchName, status, outOfStock, currentPage * pagesize, pagesize);

		Map<Category, Long> categoriesQty = new LinkedHashMap<Category, Long>();
		for (Category c : csv.getAllCategories()) {
			categoriesQty.put(c, psv.adminCountProducts(c.getId(), searchName, status, outOfStock));
		}

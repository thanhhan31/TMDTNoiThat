...
<title>TRANG THÔNG ĐƠN HÀNG</title>
<fmt:setLocale value="vi_VN" />

<div class="row">
	<div class="col align-self-center">
		<h2 class="h-100">Đơn hàng</h2>
	</div>
								...
								<div class="col-4 text-right">
									<c:choose>
										...
										<c:when test="${o.getStatus() == 1}">
											<h5 class="card-title">Đã xác nhận thanh toán</h5>
										</c:when>
						...
						<div class="card-footer">
							<div class="row">
								...
								<div class="col text-right">
									...
									<c:if test="${o.getStatus() == 2 || o.getStatus() == 1}">
							...
							<c:if test="${o.getStatus() == 0}">
								<div class="row">
									<div class="col">
										<button class="btn btn-danger float-right ml-2" onclick="cancelOrder(${o.getId()}, this)">Hủy đơn hàng</button>
										<button class="btn btn-success float-right" onclick="showPaymentModal(${o.getId()}, this)">Thanh toán</button>
									</div>
			               		</div>
		               		</c:if>
			           </div>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="card">
					<div class="card-header">
						<h5><i class="far fa-address-card mr-1"></i>Thông tin đơn hàng</h5>
					</div>
					<div class="card-body">
						<p id="paymentModalOrderInfo"></p>
						<p id="paymentModalOrderTotalPrice"></p>
					</div>
				</div>
				<form action="${pageContext.request.contextPath}/api/processpayment" method="post">
					<div class="card mt-2">
						<div class="card-header">
							<h5><i class="far fa-address-card mr-1"></i>Địa chỉ nhận hàng</h5>
						</div>
						<div class="card-body">
					...
					<div class="card mt-2">
						<div class="card-header">
							<h5><i class="fas fa-money-bill-wave mr-1"></i>Phương thức thanh toán</h5>
						</div>
						<div class="card-body">
...
<script type="text/javascript">	
	function cancelOrder(idOrder, btn) {
		$.ajax({
			url: contextPath + '/api/order/cancelorder',
			type: 'POST',
			data: { "idOrder": idOrder },
			dataType: "json",
			success: function (respone) {
				if (respone.status !== "200") {
					$("#modalErrorMessageTitle")[0].innerText = "Lỗi khi hủy đơn hàng";
					$("#modalErrorMessage")[0].innerText = respone.message;
					$("#errorModal").modal('show');
				}
				else {
					btn.closest('.card').getElementsByClassName('card-title')[1].innerText = "Đã hủy";
					btn.closest('.row').remove();
					$("#modalInfoMessageTitle")[0].innerText = "Thành công";
					$("#modalInfoMessage")[0].innerText = "Hủy đơn hàng thành công";
	...
	function showPaymentModal(idOrder, btn){
		$('#paymentModalOrderInfo')[0].innerText = btn.closest('.card').getElementsByClassName('card-title')[0].innerText;
		$('#paymentModalOrderTotalPrice')[0].innerText= btn.closest('.card-footer').getElementsByClassName('row')[0].getElementsByClassName('col')[1].innerText
		$('#idOrder')[0].value = idOrder;
